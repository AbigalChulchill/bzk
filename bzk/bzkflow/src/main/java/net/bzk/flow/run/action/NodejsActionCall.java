...
@Service("net.bzk.flow.model.flow.Action$NodejsAction")
@Scope("prototype")
public class NodejsActionCall extends ActionCall<NodejsAction> {
	...
	private void runCmd(Consumer<String> c, String... cmd) {
		int ec = ProcessUtils.exec(projectDir, c, cmd);
		if (ec != 0)
			throw new BzkRuntimeException(
					CommUtils.toJson(getModel()) + " , " + CommUtils.toJson(getUids()) + " exit code=" + ec);
	}
	...
	public String getPackageBody() {
		...
		ans = StrPlacer.build(ans).place("dependencies", CommUtils.toJson(getModel().getDependencies()))
				.place("devDependencies", CommUtils.toJson(getModel().getDevDependencies())).replace();
		return ans;
	}
	...
	public String getAppJsBody() {
		...
		ro.setUids(getUids());
		ans = StrPlacer.build(ans).place("initJson", CommUtils.toJson(ro)).place("content", getModel().getCode())
				.replace();
		return ans;
	}
